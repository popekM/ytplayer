...
export class ProviderService {
  ...
  // search song query execution
  execQ(q, results, token) {
    ...
    request.execute(function(response: any) {
      ...
      if (response.prevPageToken) {
        tempSongs.prev = response.prevPageToken;
      }
      if (response.nextPageToken) {
        tempSongs.next = response.nextPageToken;
      }
  ...
  // parameters:
  //            s: shuffle:
  //              0 - next song, 1 - random pick
  //            r: repeat:
  //              0 - don't repeat playlist, 1 - repeat whole playlist, 2 - repeat 1 song
  ...
  deletePlaylist(i) {
    ...
    if (this.data.indexOf(this.activePlaylistState) === (-1)) {
      if (this.data.length > 0) {
        ...
        this.addPlaylist({name: 'Default playlist', color: '#7ce8ff', tracks: []});
      }
    }
  }

  // Change songs order in playlist
  ...
  localStorageGetData() {
    ...
    if (temp == null) {
      ...
      this.data = temp;
      if(this.data.length==0){
        this.data = [{name: 'Default playlist', color: '#7ce8ff', tracks: []}];
      }
    }
    this.activePlaylistState = this.data[0];
    this.activePlaylist.next(this.activePlaylistState);
